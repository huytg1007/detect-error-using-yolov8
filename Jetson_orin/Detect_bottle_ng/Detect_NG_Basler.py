...
model_bottleneck = YOLO("..\\Model\\best_co_chai_2.engine")
model_body = YOLO("..\\Model\\best_than_4.engine")
...
def calculate_ng_area_worker(
    ...
    finder = CalculateNGArea(
        cropped_image, threshold_value, 1, camera_name, res_value, count
    )
    _, _, distance = finder.show_result()

    if camera_name == "Basler 1" and distance > 10:
        is_any_ng_bigger_than_accept_camera_1 = True
        # print(f"Basler 1 have ng {distance}")

    if camera_name == "Basler 2" and distance > 3:
        is_any_ng_bigger_than_accept_camera_2 = True
        # print(f"Basler 2 have ng {distance}")

    cv2.putText(
        annotated_frame,
        f"width: {int(distance)}",
        (x2 + 10, y2 + 10),
                ...
                if grabResult1.GrabSucceeded():
                    ...
                    if "1" in str(is_running) or init_basler_1 == True:
                        ...
                        if is_stop_because_lost_connect == False:
                            ...
                            cv2.putText(
                                annotated_frame,
                                f"Total ng: {total_body_ng + total_bottle_neck_ng}",
                                (0, 200),
                                cv2.FONT_HERSHEY_PLAIN,
                                2,
                                (0, 0, 255),
...
def cirle_detect_result():
    global check_result_of_one_circle
    global send_lock, conn, is_stop_because_lost_connect
    global is_ng_in_circle, is_ng_in_body, is_ng_in_bottleneck
    global total_ok, total_body_ok, total_bottle_neck_ok
    global is_any_ng_bigger_than_accept_camera_1, is_any_ng_bigger_than_accept_camera_2
    # global is_ok_in_circle
    ...
    if is_stop_because_lost_connect == False:
        if (
            ...
            and (
                is_ng_in_circle == True
                or is_ng_in_body == True
                or is_ng_in_bottleneck == True
            )
        ...
        if (
            ...
            # and is_ng_in_body == False
            # and is_ok_in_circle == True
            and check_result_of_one_circle == True
        ):
            conn.send_data_to_plc("DM", device_plc_total_ok, ".D", total_ok)
            # conn.send_data_to_plc("DM", device_plc_body_ok, ".D", total_bottle_neck_ok)
        ...
        # if (
        #     "1" not in is_running
        #     and is_ng_in_bottleneck == False
        #     # and is_ok_in_circle == True
        #     and check_result_of_one_circle == True
        # ):
        #     conn.send_data_to_plc("DM", device_plc_total_ok, ".D", total_ok)
        #     conn.send_data_to_plc(
        #         "DM", device_plc_bottleneck_ok, ".D", total_bottle_neck_ok
        #     )
        #     conn.send_data_to_plc("DM", device_plc_send_result, ".D", 2)
        #     total_ok += 1
        #     total_bottle_neck_ok += 1
        #     # is_ok_in_circle = False
        #     check_result_of_one_circle = False
        #     print("reset circle ok")
        #     print("")
...
def check_connect_to_plc():
    global send_lock, client, conn, plc_host_ip
    global is_stop_because_lost_connect
    while True:
        try:
            time.sleep(1)
            check = ping(plc_host_ip)
            if check:
